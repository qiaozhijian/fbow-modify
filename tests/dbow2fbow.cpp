...
namespace fbow {
    ...
    public:
        ...
        static void convert(ORBVocabulary &voc, fbow::Vocabulary &out_voc) {
            ...
            out_voc.setParams(8, voc.m_k, CV_8UC1, 32, nonLeafNodes, voc.getDepthLevels(), voc.size(), "orb");
            cerr << "creating     size=" << out_voc._params._total_size / (1024 * 1024) << "Mb "
                 << out_voc._params._total_size << " bytes" << endl;

            for (int i = 0; i < voc.m_nodes.size(); i++) {
                ORBVocabulary::Node &node = voc.m_nodes[i];
                if (!node.isLeaf()) {
                    auto &n_info = nodeid_info[node.id];
                    fbow::Vocabulary::Block<char> binfo = out_voc.getBlock(n_info.block);
