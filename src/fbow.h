...
// using Data_ptr = std::unique_ptr<char[], decltype(&AlignedFree)>;
    ...
    public:
        ...
        int32_t getL() const { return _params._m_L; }

        //indicates whether this object is valid
        bool isValid() const { return _data.get() != nullptr; }

        //total number of blocks
        size_t blockSize() const { return _params._nblocks; }

        size_t leafSize() const { return _params._m_nLeafs; }
    ...
    public:
        void setParams(int aligment, int k, int desc_type, int desc_size, int nblocks,
                       int L, int nLeafs, std::string desc_name);
        ...
        struct params {
            ...
            int32_t _m_L = 0; //level of three
            uint32_t _m_nLeafs = 0; //number of leafs
        };
        params _params;
        ...
        struct Block {
            ...
            Block(T *bsptr, uint64_t ds, uint64_t ds_wp, uint64_t fo, uint64_t co) : _blockstart(bsptr),
                                                                                     _desc_size_bytes(ds),
                                                                                     _desc_size_bytes_wp(ds_wp),
                                                                                     _feature_off_start(fo),
                                                                                     _child_off_start(co) {}

            Block(uint64_t ds, uint64_t ds_wp, uint64_t fo, uint64_t co) : _desc_size_bytes(ds),
                                                                           _desc_size_bytes_wp(ds_wp),
                                                                           _feature_off_start(fo),
                                                                           _child_off_start(co) {}
            ...
            inline conditional_const<block_node_info> *getBlockNodeInfo(int i) const {
                return (conditional_const<block_node_info> *) (_blockstart + _child_off_start +
                                                               i * sizeof(block_node_info));
            }

            inline void setFeature(int i, const cv::Mat &feature) {
                memcpy(_blockstart + _feature_off_start + i * _desc_size_bytes_wp, feature.ptr<char>(0),
                       feature.elemSize1() * feature.cols);
            }
            ...
            template<typename TT>
            inline conditional_const<TT> *getFeature(int i) {
        ...
        inline Block<const char> getBlock(uint32_t b) const {
            ...
            return Block<const char>(const_cast<const char *>(_data.get()) + b * _params._block_size_bytes_wp,
                                     _params._desc_size, _params._desc_size_bytes_wp, _params._feature_off_start,
        ...
        inline Block<char> getBlock(uint32_t b) {
            assert(_data != nullptr);
            assert(b < _params._nblocks);
            return Block<char>(_data.get() + b * _params._block_size_bytes_wp, _params._desc_size,
                               _params._desc_size_bytes_wp, _params._feature_off_start, _params._child_off_start);
        }

        //given a block already create with getBlock, moves it to point to block b
        template<typename T>
        inline void setBlock(uint32_t b, Block<T> &block) const {
