 ...
 */
    ...
    public:
        struct Params {
            ...
            Params(uint32_t K, int32_t l = -1, uint32_t Nthreads = 1, int MaxIters = -2) : k(K), L(l),
                                                                                           nthreads(Nthreads) {
        ...
        void create(fbow::Vocabulary &Voc, const std::vector<cv::Mat> &features, const std::string &desc_name,
                    Params params);
    ...
    private:
        ...
        struct Node {
            ...
            Node(uint32_t Id, uint32_t Parent, const cv::Mat &Feature,
                 uint32_t Feat_idx = std::numeric_limits<uint32_t>::max()) : id(Id), parent(Parent), feature(Feature),
                                                                             feat_idx(Feat_idx) {
        ...
        void assignToClusters(const std::vector<uint32_t> &findices, const std::vector<cv::Mat> &center_features,
