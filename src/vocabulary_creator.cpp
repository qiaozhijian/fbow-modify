 ...
 */
    ...
    void VocabularyCreator::create(fbow::Vocabulary &Voc, const cv::Mat &features, const std::string &desc_name,
                                   Params params) {
    ...
    void VocabularyCreator::create(fbow::Vocabulary &Voc, const std::vector<cv::Mat> &features, const string &desc_name,
                                   Params params) {
...
//ready to be threaded using producer consumer
    void VocabularyCreator::createLevel(int parent, int curL, bool recursive) {
        ...
        new_nodes.reserve(center_features.size());
        {
            for (size_t c = 0; c < center_features.size(); c++)
                new_nodes.push_back(Node(parent * _params.k + 1 + c, parent, center_features[c],
                                         findices.size() == center_features.size() ? findices[c]
    ...
    std::vector<uint32_t> VocabularyCreator::initialClusterCentersKmpp(const std::vector<uint32_t> &findices) {
        ...
        while ((int) centers.size() < _params.k) {
            ...
            double dist_sum = std::accumulate(findices.begin(), findices.end(), 0.0,
                                              [&](float acc, const unsigned fid) {
                                                  return acc + _features(fid).m_Dist;
                                              });
            if (dist_sum > 0) {
                double cut_d;
                do {
    ...
    void VocabularyCreator::assignToClusters(const std::vector<uint32_t> &findices,
...
#pragma omp parallel for
    ...
    void VocabularyCreator::convertIntoVoc(Vocabulary &Voc, std::string desc_name) {
        ...
        Voc.setParams(aligment, _params.k, _descType, _descNBytes, nonLeafNodes,
                      _params.L, nLeafNodes, desc_name);
...
//for orb
    float VocabularyCreator::distance_hamming_32bytes(const cv::Mat &a, const cv::Mat &b) {
        ...
        return uint64_popcnt(pa[0] ^ pb[0]) + uint64_popcnt(pa[1] ^ pb[1]) + uint64_popcnt(pa[2] ^ pb[2]) +
               uint64_popcnt(pa[3] ^ pb[3]);
    }
